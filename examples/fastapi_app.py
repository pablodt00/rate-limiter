...
def create_app(backend: RateLimiterBackend) -> FastAPI:
    """Build the example app on any backend (the E2E tests pass a Redis one)."""
    ...
    per_api_key = RateLimiter(
        FixedWindowCounter(limit=3, window_seconds=10), backend, key_prefix="key"
    )

    app = FastAPI(title="rate-limiter example")
    app.add_middleware(
        RateLimitMiddleware, limiter=per_ip, key_func=by_ip, exempt_paths=["/health"]
    )
    ...
    @app.get(
        "/limited",
        dependencies=[Depends(rate_limit(per_api_key, key_func=by_header("X-API-Key")))],
    )
...
app = create_app(InMemoryBackend())
